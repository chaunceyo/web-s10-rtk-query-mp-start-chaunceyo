...
import { useGetQuotesQuery,
  useToggleQuoteMutation,
  useDeleteQuoteMutation} from '../state/quotesApi'

export default function Quotes() {
  const {data : quotes, isLoading : loadingQuotes, isFetching : fetchingQuotes} = useGetQuotesQuery()
  const [toggleFake,{ isLoading: togglingQuote}] = useToggleQuoteMutation()
  ...
  return (
    <div id="quotes">
      <h3>
        Quotes
        {deletingQuote && ' being deleted...'}
        {togglingQuote && ' being toggled...'}
        {(loadingQuotes || fetchingQuotes) && ' loading...'}
      </h3>
      <div>
        {
          quotes?.filter(qt => {
